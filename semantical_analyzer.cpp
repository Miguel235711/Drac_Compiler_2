...
void SemanticalAnalyzer::create_and_print_symbol_table_and_extend_syntactical_tree(std::function<void(std::string)> & f_out){
    ...
    Scopes scopes;
    create_and_print_symbol_table_and_extend_syntactical_tree(syntactical_tree_root,NULL,f_out,none,scopes); //mode -> does not matter initial value but could be dangerous because it depends on the grammar
}
void SemanticalAnalyzer::create_and_print_symbol_table_and_extend_syntactical_tree(SyntacticalNode * node,SyntacticalNode * parent,std::function<void(std::string)> & f_out,Mode mode,Scopes & scopes){
    ...
    if((symbol==LexicalAnalyzer::curly_open &&parent&&parent->symbol!=LexicalAnalyzer::fun_def)|| symbol == LexicalAnalyzer::fun_def || symbol == LexicalAnalyzer::curly_close){
    ...
    if(symbol==LexicalAnalyzer::id_symbol){
        ...
        if(mode == var_def || mode == fun_def){
            ...
            auto was_insertion_successful = scopes.try_insert_in_scope(index,content,id_node);
    ...
    for(auto adjacent: node->adjacent)
        create_and_print_symbol_table_and_extend_syntactical_tree(adjacent,node,f_out,next_mode,scopes);
